...
def parse_args():
    ...
    parser.add_argument('-graph', nargs='?', default='data/wikipedia.ungraph',
                        help='Graph path')
    ...
    parser.add_argument('-label', nargs='?', default='data/wikipedia.cmty',
...
t_0 = time.time()
model = AROPE(args.graph, args.dimension)
t_1 = time.time()
...
#eigenvalues = EigenValues[:,0:d]/np.sum(EigenValues[:,0:d])
...
Activation0 = np.true_divide(np.sum(yl,1), NumberOfNodes/hid)+0.3

Activation0[Activation0>0.7] = 0.7
np.save('activation_wiki.npy', Activation0)
